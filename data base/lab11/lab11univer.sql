...
DECLARE discpipline CURSOR for select SUBJECT_NAME from SUBJECT where PULPIT like 'ИСиТ';
OPEN discpipline;
...
/* ex_2 */
DECLARE Puplit_cursor CURSOR GLOBAL for select FACULTY from PULPIT;
...
DECLARE Puplit_cursor_Local CURSOR LOCAL for select FACULTY from PULPIT;
--deallocate Puplit_cursor_Local;
DECLARE @pupl char(50), @pupls char(100) ='';
OPEN Puplit_cursor_Local;
FETCH  Puplit_cursor_Local into @pupl;
	set @pupls ='1. ' + RTRIM(@pupl);	
	print @pupls;
CLOSE Puplit_cursor_Local;
...
-- ex_4 
DECLARE @number varchar(6), @idgroup varchar(6), @faculty varchar(10), @profession varchar(6), @pdate varchar (11);
DECLARE GROUPS_CURSOR_SCROLL CURSOR LOCAL DYNAMIC SCROLL
	for select ROW_NUMBER() over (order by IDGROUP) Номер,
	* from GROUPS;

OPEN GROUPS_CURSOR_SCROLL
FETCH GROUPS_CURSOR_SCROLL into @number, @idgroup, @faculty, @profession, @pdate;
print 'Первая выбранная строка: ' + CHAR(10) +
'Номер записи: '+ rtrim(@number) +
' ID: '+ rtrim(@idgroup)  +
'. Факультет: '+ rtrim(@faculty) +
'. Специальность: ' + rtrim(@profession) +
'. Год: '  + rtrim(@pdate);

FETCH LAST from GROUPS_CURSOR_SCROLL into @number, @idgroup, @faculty, @profession, @pdate;
...
FETCH RELATIVE -1  from GROUPS_CURSOR_SCROLL into @number, @idgroup, @faculty, @profession, @pdate;
...
FETCH ABSOLUTE 2  from GROUPS_CURSOR_SCROLL into @number, @idgroup, @faculty, @profession, @pdate;
...
FETCH ABSOLUTE -3 from GROUPS_CURSOR_SCROLL into @number, @idgroup, @faculty, @profession, @pdate;
