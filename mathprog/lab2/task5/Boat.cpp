...
namespace combi
{
    bool compv(combi::accomodation s, const int ming[], //проверка допустимости размещения 
    ...
    int calcc(combi::accomodation s, const int c[]) //вычесление дохода
    {
        int rc = 0;
        for (int i = 0; i < s.m; i++) rc += c[s.ntx(i)];
        return rc;
    };
    void copycomb(short m, short* r1, const short* r2) //сохронение ниболее подходящего размещения
    {
